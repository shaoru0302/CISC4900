...
function renderBracelets(products) {
  ...
  if (!products.length) {
    productGrid.innerHTML =
      '<p class="product-description" style="grid-column:1/-1;">No bracelet products found. Add rows with <code>category = bracelet</code> or import project seed CSV.</p>';
    return;
  }
...
fetch(`${origin}/api/products?category=bracelet`)
  .then((response) => {
    if (!response.ok) {
  ...
  .then((data) => {
    ...
    renderBracelets(Array.isArray(data) ? data : []);
  })
  .catch((error) => {
    console.error("Fetch bracelet products error:", error);
    const productGrid = document.getElementById("product-grid");
    if (productGrid) {
      productGrid.innerHTML =
